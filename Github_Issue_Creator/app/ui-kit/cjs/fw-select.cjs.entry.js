...
const Select = class {
    ...
    renderTags() {
        if (this.multiple) {
            return this.options
                ...
                .map(option => core.h("fw-tag", { text: option.text, value: option.value }));
        }
    }
    renderDropdown() {
        return this.filteredOptions.map(option => (core.h("fw-select-option", { value: option.value, selected: option.selected }, option.text)));
    }
    componentWillLoad() {
    ...
    render() {
        return (core.h(core.Host, { "aria-disabled": this.disabled, class: {
            ...
            } }, this.label !== '' ? core.h("label", { class: { 'required': this.required } }, " ", this.label, " ") : '', core.h("div", { class: "select-container" }, core.h("div", { class: "input-container", ref: select => this.select = select, onClick: () => this.innerOnClick() }, core.h("div", { class: {
                'input-container-inner': true,
                [this.state]: true,
            } }, this.renderTags(), core.h("input", { ref: selectInput => this.selectInput = selectInput, class: {
                'multiple-select': this.multiple,
            }, autoComplete: "off", disabled: this.disabled, name: this.name, placeholder: this.placeholder || '', readOnly: this.readonly, required: this.required, type: this.type, value: "", onInput: () => this.onInput(), onFocus: e => this.innerOnFocus(e), onBlur: e => this.innerOnBlur(e) }), core.h("span", { class: { 'dropdown-status-icon': true, 'expanded': this.isExpanded } }))), core.h("ul", { tabindex: "0", ref: ul => this.selectList = ul }, this.renderDropdown()), this.stateText !== '' ?
